...
test('第三幕最終站保留短 dwell，但不再產生一秒卡死感', () => {
  const { controller } = createController()
  const finalIndex = journeyStations.points.length - 1
  const finalDelay = journeyStations.points[finalIndex].rearmDelay
  controller.enter(finalIndex, 100)

  assert.ok(finalDelay < 1000)
  assert.equal(controller.update(100 + finalDelay - 1).armed, false)
  assert.equal(controller.update(100 + finalDelay).armed, true)
})
